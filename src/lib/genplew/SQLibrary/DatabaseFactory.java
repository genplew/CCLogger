...
import lib.genplew.SQLibrary.DatabaseConfig.Parameter;
	...
	public static Database createDatabase(DatabaseConfig config) throws InvalidConfiguration {
		if (!config.isValid())
			throw new InvalidConfiguration(
					"The configuration is invalid, you don't have enought parameter for that DB : " + config.getType());
		switch (config.getType()) {
		case MYSQL:
			return new MySQL(config.getLog(), config.getParameter(Parameter.DB_PREFIX),
					config.getParameter(Parameter.HOSTNAME), config.getParameter(Parameter.PORT_NUMBER),
		...
		case SQLITE:
			return new SQLite(config.getLog(), config.getParameter(Parameter.DB_PREFIX),
					config.getParameter(Parameter.DB_NAME), config.getParameter(Parameter.DB_LOCATION));
		default:
			return null;
		}
	}
